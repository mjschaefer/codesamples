...
class Street_Controller extends Template_Controller {
  ...
  public function index($url = 'street_department')
  {
    // Grab the Town Alerts
	  $alert = ORM::factory('alert')->find();
	  
	  // Grab links for submenu.
	  $page_list = ORM::factory('page')->select('title, url')->where('section', 'street')->orderby('order', 'ASC')->find_all();
	  
	  // Try to grab our page.
	  $page = ORM::factory('page')->where( array('section' => 'street', 'url' => $url) )->find();
	  
	  if($page->loaded == true)
	  {
	    // Setup our view.
  		$view = new View('street');
  		$view->page = $page;
	  ...
	  else
	  {
	    ...
	    $page = ORM::factory('page')->where( array('section' => 'street', 'url' => 'street_department') )->find();
