...
class Water_Controller extends Template_Controller {
  ...
  public function index($url = 'water_department')
  {
    // Grab the Town Alerts
	  $alert = ORM::factory('alert')->find();
	  
	  // Grab links for submenu.
	  $page_list = ORM::factory('page')->select('title, url')->where('section', 'water')->orderby('order', 'ASC')->find_all();
	  
	  // Try to grab our page.
	  $page = ORM::factory('page')->where( array('section' => 'water', 'url' => $url) )->find();
	  ...
	  else
	  {
	    ...
	    $page = ORM::factory('page')->where( array('section' => 'water', 'url' => 'water_department') )->find();
