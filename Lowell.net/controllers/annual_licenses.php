...
class Annual_Licenses_Controller extends Template_Controller {
  ...
  public function index($url = 'annual_licenses')
  {
    // Grab the Town Alerts
	  $alert = ORM::factory('alert')->find();
	  
	  // Grab links for submenu.
	  $page_list = ORM::factory('page')->select('title, url')->where('section', 'licenses')->orderby('order', 'ASC')->find_all();
	  
	  // Try to grab our page.
	  $page = ORM::factory('page')->where( array('section' => 'licenses', 'url' => $url) )->find();
	  
	  if($page->loaded == true)
	  {
	    // Setup our view.
  		$view = new View('licenses');
  		$view->page = $page;
	  ...
	  else
	  {
	    ...
	    $page = ORM::factory('page')->where( array('section' => 'licenses', 'url' => 'annual_licenses') )->find();
