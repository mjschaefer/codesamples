...
class Council_Controller extends Template_Controller {
  ...
  public function index($url = 'town_council')
  {
    // Grab the Town Alerts
	  $alert = ORM::factory('alert')->find();
	  
	  // Grab links for submenu.
	  $page_list = ORM::factory('page')->select('title, url')->where('section', 'council')->orderby('order', 'ASC')->find_all();
	  
	  // Try to grab our page.
	  $page = ORM::factory('page')->where( array('section' => 'council', 'url' => $url) )->find();
	  
	  if($page->loaded == true)
	  {
	    // Setup our view.
  		$view = new View('council');
  		$view->page = $page;
	  ...
	  else
	  {
	    ...
	    $page = ORM::factory('page')->where( array('section' => 'council', 'url' => 'town_council') )->find();
  ...
  public function reports($year = '', $type = '')
  {
    // Grab the Town Alerts
	  ...
	  $reports = ORM::factory('report')->where( array('section' => 'council', 'year' => $year, 'type' => $type) )->orderby('date', 'DESC')->find_all();
	  ...
	  {
	    $minute_years = ORM::factory('report')->select('DISTINCT ' . 'year')->where( array('section' => 'council', 'type' => 'minutes') )->orderby('year', 'DESC')->find_all();
	    $agenda_years = ORM::factory('report')->select('DISTINCT ' . 'year')->where( array('section' => 'council', 'type' => 'agendas') )->orderby('year', 'DESC')->find_all();
  ...
  public function minutes($month, $day, $year)
  {
    ...
    $minutes = ORM::factory('report')->where( array('section' => 'council', 'type' => 'minutes', 'month' => $month, 'day' => $day, 'year' => $year) )->find();
    
    if($minutes->loaded == false)
    {
      url::redirect('council');
    }
    else
    {
  ...
  public function agendas($month, $day, $year)
  {
    ...
    $agenda = ORM::factory('report')->where( array('section' => 'council', 'type' => 'agendas', 'month' => $month, 'day' => $day, 'year' => $year) )->find();
